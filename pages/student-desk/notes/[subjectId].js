...
export default function SubjectNotesPage() {
  ...
  // Derived value — must be declared before any hook below references it,
  // and before any early `return`, so hook order stays stable across renders.
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      // PDFs are stored in the top-level pdfs collection in admin.
      // Some deployments don't have the composite index for subjectId + createdAt,
      // so we fall back to a client-side filter and sort when needed.
  ...
  useEffect(() => {
    ...
    const html = note || defaultNoteTemplate(name);
