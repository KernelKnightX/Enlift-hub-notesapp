...
export default function BiosphereReservesAdmin() {
  ...
  return (
    <AdminLayout
      title="Biosphere Reserves · Maps"
      subtitle={`${maps.length} reserves`}
      backHref="/admin/maps"
    >
      {maps.length === 0 ? (
        <div className="card p-12 text-center">
          ...
          <Link
            href="/admin/maps/biosphere-reserves/new"
        ...
        <div className="card overflow-hidden">
          <div className="hairline-b bg-[var(--color-surface-alt)] px-5 py-3">
            <div className="text-[13px] font-semibold text-[var(--color-ink-2)]">
              Biosphere Reserves ({maps.length})
            </div>
          </div>
          <div>
            {maps.map((item, index) => (
      ...
      <div className="mt-4 flex gap-2">
        ...
        <Link
          href="/admin/maps/import?category=biosphere-reserves"
          className="btn"
        >
          Import CSV
        </Link>
