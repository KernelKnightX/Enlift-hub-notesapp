      ...
      <form onSubmit={handleSubmit} className="card p-6">
        ...
        <div className="grid gap-4 sm:grid-cols-2 mt-4">
          ...
          <div>
            ...
            <input className="map-form-input" value={origin} onChange={(e) => setOrigin(e.target.value)} />
          </div>
          <div>
            <label className="block mb-1">Mouth</label>
            <input className="map-form-input" value={mouth} onChange={(e) => setMouth(e.target.value)} />
          </div>
          <div>
            <label className="block mb-1">Length (km)</label>
            <input className="map-form-input" value={lengthKm} onChange={(e) => setLengthKm(e.target.value)} />
          </div>
          <div>
            <label className="block mb-1">States Covered</label>
            <input className="map-form-input" value={statesCovered} onChange={(e) => setStatesCovered(e.target.value)} />
          </div>
          <div className="sm:col-span-2">
            <label className="block mb-1">Tributaries</label>
            <input className="map-form-input" value={tributaries} onChange={(e) => setTributaries(e.target.value)} />
          </div>
        </div>
        ...
        <div className="mt-4 flex justify-end gap-2">
          <button type="button" className="btn" onClick={() => router.push('/admin/maps/river-systems')}>Cancel</button>
          <button type="submit" className="btn btn-primary" disabled={saving}>{saving ? 'Saving...' : 'Create River'}</button>
        </div>
      </form>
    </AdminLayout>
  );
