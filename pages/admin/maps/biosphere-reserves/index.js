...
export default function BiosphereReservesAdmin() {
  const { loading, isAdmin } = useAdminGate();
  const [maps, setMaps] = useState([]);
  const CATEGORY = 'biosphere-reserves';
  ...
  return (
    <AdminLayout title="Biosphere Reserves · Maps" subtitle={`${maps.length} reserves`} backHref="/admin/maps">
      {maps.length === 0 ? (
        <div className="card p-12 text-center">
          <Map className="mx-auto mb-3 text-[var(--color-primary)]" size={32} strokeWidth={1.6} />
          <div className="font-semibold text-[var(--color-ink)]">No reserves yet.</div>
          <Link href="/admin/maps/biosphere-reserves/new" className="btn btn-primary mt-5">Add Reserve</Link>
        </div>
      ) : (
        <div className="card overflow-hidden">
          <div className="hairline-b bg-[var(--color-surface-alt)] px-5 py-3">
            <div className="text-[13px] font-semibold text-[var(--color-ink-2)]">Biosphere Reserves ({maps.length})</div>
          </div>
          <div>
            {maps.map((item, index) => (
              <div key={item.id} className={`grid gap-4 px-5 py-4 md:grid-cols-[72px_1fr_auto] md:items-center ${index !== maps.length - 1 ? "hairline-b" : ""}`}>
                ...
                <div className="min-w-0">
                  <div className="flex flex-wrap items-center gap-2">
                    <span className="chip chip-primary">Biosphere Reserves</span>
      ...
      <div className="mt-4 flex gap-2">
        <Link href="/admin/maps/biosphere-reserves/new" className="btn btn-primary">Add Reserve</Link>
        <Link href="/admin/maps/import?category=biosphere-reserves" className="btn">Import CSV</Link>
      </div>
    </AdminLayout>
  );
}
