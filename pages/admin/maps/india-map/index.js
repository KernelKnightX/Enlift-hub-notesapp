...
export default function IndiaMapAdmin() {
  const { loading, isAdmin } = useAdminGate();
  const [maps, setMaps] = useState([]);
  const CATEGORY = "india-states";
  ...
  return (
    <AdminLayout
      title="India Map · Maps"
      subtitle={`${maps.length} India state maps`}
      backHref="/admin/maps"
    >
      {maps.length === 0 ? (
              ...
              >
                ...
                <div className="min-w-0">
                  <div className="flex flex-wrap items-center gap-2">
                    <span className="chip chip-primary">India States</span>
                    <span
      ...
      <div className="mt-4 flex gap-2">
        <Link href="/admin/maps/india-map/new" className="btn btn-primary">
          Add India Map
        </Link>
        <Link href="/admin/maps/import?category=india-states" className="btn">
          Import CSV
        </Link>
      </div>
    </AdminLayout>
