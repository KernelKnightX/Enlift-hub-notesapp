    ...
    <AdminLayout title="Add World Map" subtitle="Create world map entry" backHref="/admin/maps/world">
      <form onSubmit={handleSubmit} className="card p-6">
        ...
        <div className="mt-4 flex justify-end gap-2">
          <button type="button" className="btn" onClick={() => router.push('/admin/maps/world')}>Cancel</button>
          <button type="submit" className="btn btn-primary" disabled={saving}>{saving ? 'Saving...' : 'Create World Map'}</button>
        </div>
      </form>
    </AdminLayout>
  );
