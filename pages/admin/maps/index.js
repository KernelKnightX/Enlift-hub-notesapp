...
export default function AdminMapsPage() {
  ...
  const counts = useMemo(() => {
    ...
    MAP_CATEGORIES.forEach((item) => {
      base[item.value] = maps.filter(
        (mapItem) => mapItem.category === item.value,
      ).length;
    });
    return base;
  }, [maps]);
  ...
  const displayed =
    filter === "all"
      ? maps
      : maps.filter((mapItem) => mapItem.category === filter);
    ...
    >
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3 mb-6">
        {MAP_CATEGORIES.map((item) => (
          <Link
            key={item.value}
            href={`/admin/maps/${item.value}`}
            className="card p-3 text-center hover:shadow"
          >
            <div
