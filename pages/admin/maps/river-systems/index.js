...
export default function RiverSystemsAdmin() {
  ...
  return (
    <AdminLayout title="River Systems · Maps" subtitle={`${maps.length} rivers`} backHref="/admin/maps">
      {maps.length === 0 ? (
        <div className="card p-12 text-center">
          <Map className="mx-auto mb-3 text-[var(--color-primary)]" size={32} strokeWidth={1.6} />
          <div className="font-semibold text-[var(--color-ink)]">No rivers yet.</div>
          <Link href="/admin/maps/river-systems/new" className="btn btn-primary mt-5">Add River System</Link>
        </div>
      ) : (
        <div className="card overflow-hidden">
          <div className="hairline-b bg-[var(--color-surface-alt)] px-5 py-3">
            <div className="text-[13px] font-semibold text-[var(--color-ink-2)]">River Systems ({maps.length})</div>
          </div>
          <div>
            {maps.map((item, index) => (
              <div key={item.id} className={`grid gap-4 px-5 py-4 md:grid-cols-[72px_1fr_auto] md:items-center ${index !== maps.length - 1 ? "hairline-b" : ""}`}>
                ...
                <div className="flex gap-2">
                  <Link href={`/admin/maps/river-systems/${item.id}/edit`} className="btn btn-ghost !px-3 !py-2 text-[13px]"><Edit3 size={14} /> Edit</Link>
      ...
      <div className="mt-4 flex gap-2">
        <Link href="/admin/maps/river-systems/new" className="btn btn-primary">Add River System</Link>
        <Link href="/admin/maps/import?category=river-systems" className="btn">Import CSV</Link>
      </div>
    </AdminLayout>
  );
}
