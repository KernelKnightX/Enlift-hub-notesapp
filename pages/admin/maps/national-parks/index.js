...
export default function NationalParksAdmin() {
  const { loading, isAdmin } = useAdminGate();
  const [maps, setMaps] = useState([]);
  const CATEGORY = "national-parks";
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(
      q,
      (snap) => setMaps(snap.docs.map((d) => ({ id: d.id, ...d.data() }))),
      (err) => {
        console.error(err);
  ...
  return (
    <AdminLayout
      title="National Parks · Maps"
      subtitle={`${maps.length} parks`}
      backHref="/admin/maps"
    >
      {maps.length === 0 ? (
        <div className="card p-12 text-center">
          ...
          <Link
            href="/admin/maps/national-parks/new"
        ...
        <div className="card overflow-hidden">
          ...
          <div>
            {maps.map((item, index) => (
              <div
                key={item.id}
                className={`grid gap-4 px-5 py-4 md:grid-cols-[72px_1fr_auto] md:items-center ${index !== maps.length - 1 ? "hairline-b" : ""}`}
              >
                ...
                <div className="min-w-0">
                  <div className="flex flex-wrap items-center gap-2">
                    <span className="chip chip-primary">National Parks</span>
                    <span
                      className={`chip ${item.status === "published" ? "chip-green" : "chip-amber"}`}
                    >
                      {item.status === "published" ? "Published" : "Draft"}
                    </span>
                  </div>
                  <div className="mt-2 text-[15px] font-semibold text-[var(--color-ink)]">
                    {item.title}
                  </div>
      ...
      <div className="mt-4 flex gap-2">
        <Link href="/admin/maps/national-parks/new" className="btn btn-primary">
          Add Park
        </Link>
        <Link href="/admin/maps/import?category=national-parks" className="btn">
          Import CSV
        </Link>
      </div>
    </AdminLayout>
