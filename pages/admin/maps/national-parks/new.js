...
export default function NewPark() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const payload = {
        ...
        parkState: parkState.trim() || undefined,
        establishedYear: establishedYear.trim() || undefined,
        parkArea: parkArea.trim() || undefined,
        famousFor: famousFor.trim() || undefined,
      ...
      router.push('/admin/maps/national-parks');
    } catch (err) {
      console.error(err);
  ...
  return (
    <AdminLayout title="Add National Park" subtitle="Create park entry" backHref="/admin/maps/national-parks">
      <form onSubmit={handleSubmit} className="card p-6">
        ...
        <div className="grid gap-4 sm:grid-cols-2 mt-4">
          ...
          <div>
            ...
            <input className="map-form-input" value={parkState} onChange={(e) => setParkState(e.target.value)} />
          </div>
          <div>
            <label className="block mb-1">Established Year</label>
            <input className="map-form-input" value={establishedYear} onChange={(e) => setEstablishedYear(e.target.value)} />
          </div>
          <div>
            <label className="block mb-1">Area</label>
            <input className="map-form-input" value={parkArea} onChange={(e) => setParkArea(e.target.value)} />
          </div>
          <div className="sm:col-span-2">
            <label className="block mb-1">Famous For</label>
            <input className="map-form-input" value={famousFor} onChange={(e) => setFamousFor(e.target.value)} />
          </div>
        </div>
        ...
        <div className="mt-4 flex justify-end gap-2">
          <button type="button" className="btn" onClick={() => router.push('/admin/maps/national-parks')}>Cancel</button>
          <button type="submit" className="btn btn-primary" disabled={saving}>{saving ? 'Saving...' : 'Create Park'}</button>
        </div>
      </form>
    </AdminLayout>
  );
