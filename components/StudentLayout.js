...
import {
  ...
  ListChecks, Calendar as CalendarIcon, User as UserIcon,
  Coffee, Menu, X, LogOut, Search, Bell, Sparkles, ChevronRight, Command, ArrowUpRight
} from 'lucide-react';
...
const NAV = [
  ...
  { label: 'Syllabus',        icon: ListChecks,      href: '/student-desk/syllabus' },
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--color-bg)', display: 'flex' }}>
      ...
      >
        ...
        {/* Plus Upgrade card */}
        {!user.isPlus && (
          <div className="mt-8 mx-1 p-4 rounded-2xl relative overflow-hidden"
               style={{ background: 'var(--color-ink)', color: 'var(--color-bg)' }}>
            <div className="chip" style={{ background: 'rgba(178,90,61,0.15)', color: '#E8A889', borderColor: 'rgba(178,90,61,0.35)' }}>
              <Sparkles size={10} /> Notes Cafe Plus
            </div>
            <div className="mt-3 font-serif text-[17px] leading-[1.2]">
              Unlock every mock, PYQ & analytic
            </div>
            <Link href="/#pricing" className="mt-4 flex items-center justify-between text-[12.5px] font-medium"
                  style={{ color: 'var(--color-accent)' }} data-testid="sidebar-upgrade">
              Start 7-day trial <ArrowUpRight size={13} />
            </Link>
          </div>
        )}
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="ml-auto flex items-center gap-2 md:gap-3">
              <div className="hidden md:flex items-center gap-2 px-3 py-2 rounded-xl"
                ...
                <input placeholder="Search notes, mocks, current affairs…"
