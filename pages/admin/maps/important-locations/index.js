...
export default function ImportantLocationsAdmin() {
  const { loading, isAdmin } = useAdminGate();
  const [maps, setMaps] = useState([]);
  const CATEGORY = 'important-locations';
  ...
  return (
    <AdminLayout title="Important Locations · Maps" subtitle={`${maps.length} locations`} backHref="/admin/maps">
      {maps.length === 0 ? (
        <div className="card p-12 text-center">
          <Map className="mx-auto mb-3 text-[var(--color-primary)]" size={32} strokeWidth={1.6} />
          <div className="font-semibold text-[var(--color-ink)]">No locations yet.</div>
          <Link href="/admin/maps/important-locations/new" className="btn btn-primary mt-5">Add Location</Link>
        </div>
      ) : (
        <div className="card overflow-hidden">
          <div className="hairline-b bg-[var(--color-surface-alt)] px-5 py-3">
            <div className="text-[13px] font-semibold text-[var(--color-ink-2)]">Important Locations ({maps.length})</div>
          </div>
          <div>
            {maps.map((item, index) => (
              <div key={item.id} className={`grid gap-4 px-5 py-4 md:grid-cols-[72px_1fr_auto] md:items-center ${index !== maps.length - 1 ? "hairline-b" : ""}`}>
                ...
                <div className="min-w-0">
                  <div className="flex flex-wrap items-center gap-2">
                    <span className="chip chip-primary">Important Locations</span>
      ...
      <div className="mt-4 flex gap-2">
        <Link href="/admin/maps/important-locations/new" className="btn btn-primary">Add Location</Link>
        <Link href="/admin/maps/import?category=important-locations" className="btn">Import CSV</Link>
      </div>
    </AdminLayout>
  );
}
