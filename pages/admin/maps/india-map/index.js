...
export default function IndiaMapAdmin() {
  ...
  return (
    <AdminLayout title="India Map · Maps" subtitle={`${maps.length} India state maps`} backHref="/admin/maps">
      {maps.length === 0 ? (
        <div className="card p-12 text-center">
          <Map className="mx-auto mb-3 text-[var(--color-primary)]" size={32} strokeWidth={1.6} />
          <div className="font-semibold text-[var(--color-ink)]">No India maps yet.</div>
          <Link href="/admin/maps/india-map/new" className="btn btn-primary mt-5">Add India Map</Link>
        </div>
      ) : (
        <div className="card overflow-hidden">
          <div className="hairline-b bg-[var(--color-surface-alt)] px-5 py-3">
            <div className="text-[13px] font-semibold text-[var(--color-ink-2)]">India States ({maps.length})</div>
          </div>
          <div>
            {maps.map((item, index) => (
      ...
      <div className="mt-4 flex gap-2">
        <Link href="/admin/maps/india-map/new" className="btn btn-primary">Add India Map</Link>
        <Link href="/admin/maps/import?category=india-states" className="btn">Import CSV</Link>
      </div>
    </AdminLayout>
  );
}
