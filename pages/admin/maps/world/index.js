...
export default function WorldAdmin() {
  ...
  return (
    <AdminLayout title="World Maps · Maps" subtitle={`${maps.length} world maps`} backHref="/admin/maps">
      {maps.length === 0 ? (
        <div className="card p-12 text-center">
          <Map className="mx-auto mb-3 text-[var(--color-primary)]" size={32} strokeWidth={1.6} />
          <div className="font-semibold text-[var(--color-ink)]">No world maps yet.</div>
          <Link href="/admin/maps/world/new" className="btn btn-primary mt-5">Add World Map</Link>
        </div>
      ) : (
        <div className="card overflow-hidden">
          <div className="hairline-b bg-[var(--color-surface-alt)] px-5 py-3">
            <div className="text-[13px] font-semibold text-[var(--color-ink-2)]">World ({maps.length})</div>
          </div>
          <div>
            {maps.map((item, index) => (
      ...
      <div className="mt-4 flex gap-2">
        <Link href="/admin/maps/world/new" className="btn btn-primary">Add World Map</Link>
        <Link href="/admin/maps/import?category=world" className="btn">Import CSV</Link>
      </div>
    </AdminLayout>
  );
}
